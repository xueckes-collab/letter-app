...
function DashboardLayoutContent({ children, setSidebarWidth }: DashboardLayoutContentProps) {
  ...
  const allMenuItems = displayUser?.role === 'admin' ? [...menuItems, ...adminMenuItems] : menuItems;
              ...
              <DropdownMenuTrigger asChild>
                <button className="flex items-center gap-3 rounded-lg px-1 py-1 hover:bg-accent/50 transition-colors w-full text-left group-data-[collapsible=icon]:justify-center focus:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                  ...
                  <div className="flex-1 min-w-0 group-data-[collapsible=icon]:hidden">
                    <p className="text-sm font-medium truncate leading-none">{displayUser?.name || "-"}</p>
                    <p className="text-xs text-muted-foreground truncate mt-1.5">{displayUser?.email || "-"}</p>
                  </div>
                </button>
              </DropdownMenuTrigger>
