...
export default function AdminDashboard() {
  const { data: stats, isLoading: statsLoading, refetch: refetchStats } = trpc.admin.getDashboardStats.useQuery();
  const { data: failedEmails, isLoading: failedLoading, refetch: refetchFailed } = trpc.admin.getRecentFailedEmails.useQuery();
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {[
              { label: "邮件发送服务", status: "正常", color: "text-emerald-500" },
              { label: "自动化调度器", status: "运行中", color: "text-emerald-500" },
              { label: "回复检测服务", status: "运行中", color: "text-emerald-500" },
              { label: "AI 生成服务",  status: "正常", color: "text-emerald-500" },
              { label: "数据库连接",   status: "正常", color: "text-emerald-500" },
              { label: "文件存储",     status: "正常", color: "text-emerald-500" },
